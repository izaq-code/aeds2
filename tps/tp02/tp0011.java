...
public class tp0011 {
    ...
    /**
     * Implementação do Counting Sort para ordenar por ano de lançamento
     */
    public static void countingSort(Show[] array, int n) {
        if (n == 0) return;

        // Encontra o menor e maior ano
        int min = array[0].getReleaseYear();
        int max = array[0].getReleaseYear();
        for (int i = 1; i < n; i++) {
            int year = array[i].getReleaseYear();
            if (year < min) min = year;
            else if (year > max) max = year;
        }
        ...
        for (int i = 0; i < n; i++) {
            count[array[i].getReleaseYear() - min]++;
            mov++;
        }

        // Cálculo das posições acumuladas
        ...
        for (int i = n - 1; i >= 0; i--) {
            int pos = --count[array[i].getReleaseYear() - min];
            output[pos] = array[i];
            mov++;
        }
        ...
        // Ordenação por título para anos iguais
        ordenarPorTitulo(array, n);
    }

    /**
     * Método para ordenar por título quando os anos são iguais
     */
    public static void ordenarPorTitulo(Show[] array, int n) {
        for (int i = 1; i < n; i++) {
            Show temp = array[i];
            int j = i - 1;

            while (j >= 0 && array[j].getReleaseYear() == temp.getReleaseYear()) {
                comp++;
                if (array[j].getTitle().compareToIgnoreCase(temp.getTitle()) > 0) {
                    array[j + 1] = array[j];
                    mov++;
                    j--;
                } else break;
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileWriter fw = new FileWriter("877487_countingsort.txt");
